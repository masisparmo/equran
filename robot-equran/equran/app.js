...
let apiKeys = []; // Gemini Keys
let apiKeysGroq = []; // Groq Keys
let currentApiKeyIndex = 0;
let currentGroqApiKeyIndex = 0;
let currentProvider = 'gemini'; // 'gemini', 'groq', or 'auto'
...
// Groq/Settings UI
const groqSettingsPane = document.getElementById('groq-settings');
const geminiSettingsPane = document.getElementById('gemini-settings');
const keysListGroq = document.getElementById('groq-keys-list');
...
function setupEventListeners() {
    ...
    addKeyBtn.addEventListener('click', () => addApiKey(newApiKeyInput.value, newApiKeyInput, 'gemini'));
    addGroqKeyBtn.addEventListener('click', () => addApiKey(newGroqKeyInput.value, newGroqKeyInput, 'groq'));
...
async function analyzeWordWithAI(wordText, surahNum, ayahNum, wordIndex, element) {
    ...
    if (currentProvider === 'gemini') {
        resultJSON = await tryProvider('gemini', apiKeys, currentApiKeyIndex, aiPrompt);
    } else if (currentProvider === 'groq') {
        resultJSON = await tryProvider('groq', apiKeysGroq, currentGroqApiKeyIndex, aiPrompt);
    } else if (currentProvider === 'auto') {
        // Try Gemini First
        resultJSON = await tryProvider('gemini', apiKeys, currentApiKeyIndex, aiPrompt);
        // Fallback to Groq if Gemini fails
        if (!resultJSON) {
            console.log("Gemini limit reached or failed. Falling back to Groq...");
            resultJSON = await tryProvider('groq', apiKeysGroq, currentGroqApiKeyIndex, aiPrompt);
        }
    }
...
async function tryProvider(type, keys, startIndex, prompt) {
    ...
    while (attempts < keys.length) {
        ...
        try {
            ...
            // Update the persistent index for the successful key
            if (type === 'gemini') currentApiKeyIndex = idx;
            else currentGroqApiKeyIndex = idx;

            return result;
        } catch (error) {
            console.error(`${type} API Error (Key Index ${idx}):`, error);
            attempts++;
        }
    }
    return null;
...
async function loadApiKeys() {
    try {
        const geminiKeys = await localforage.getItem('gemini_api_keys');
        if (geminiKeys && Array.isArray(geminiKeys)) apiKeys = geminiKeys;

        const groqKeys = await localforage.getItem('groq_api_keys');
        if (groqKeys && Array.isArray(groqKeys)) apiKeysGroq = groqKeys;
    } catch (e) {
...
async function saveApiKeys(type) {
    try {
        const storageKey = type === 'gemini' ? 'gemini_api_keys' : 'groq_api_keys';
        const keysToSave = type === 'gemini' ? apiKeys : apiKeysGroq;
        await localforage.setItem(storageKey, keysToSave);
    } catch (e) {
...
async function loadProviderSettings() {
    try {
        const provider = await localforage.getItem('current_provider');
        if (provider) {
...
function saveProviderSettings() {
    localforage.setItem('current_provider', currentProvider);
}

function updateSettingsPaneVisibility() {
    if (currentProvider === 'groq') {
        geminiSettingsPane.style.display = 'none';
        groqSettingsPane.style.display = 'block';
    } else {
        geminiSettingsPane.style.display = 'block';
        groqSettingsPane.style.display = 'none';
    }
}

function addApiKey(inputValue, inputElement, type) {
    const rawInput = inputValue.trim();
    if (!rawInput) return false;

    const keysToAdd = rawInput.split(',').map(k => k.trim()).filter(k => k.length > 0);
    const targetArray = type === 'gemini' ? apiKeys : apiKeysGroq;
    ...
    if (addedCount > 0) {
        ...
        renderKeys(type);
        inputElement.value = '';
        alert(`${addedCount} API Key ${type} berhasil ditambahkan!`);
        return true;
    }
    return false;
}

function removeApiKey(index, type) {
    const targetArray = type === 'gemini' ? apiKeys : apiKeysGroq;
...
function renderKeys(type) {
    const listEl = type === 'gemini' ? keysList : keysListGroq;
    const targetArray = type === 'gemini' ? apiKeys : apiKeysGroq;

    listEl.innerHTML = '';
    if (targetArray.length === 0) {
        listEl.innerHTML = `<li>Belum ada API Key ${type} tersimpan.</li>`;
        return;
    }

    targetArray.forEach((key, index) => {
        const li = document.createElement('li');
        const maskedKey = key.substring(0, 4) + '...' + key.substring(key.length - 4);
        li.innerHTML = `
            <span>${maskedKey}</span>
            <button onclick="removeApiKey(${index}, '${type}')" title="Hapus"><i class="fas fa-trash"></i></button>
        `;
